...
\begin{itemize}
    ...
    \item $-E\bigr[\frac{\partial^{2} l_{c,(i,g)}}{\partial \gamma_{g}\gamma_{g}^{\mathsf{T}}}\bigr] = X^{\mathsf{T}}P_{1}(\beta_{g}^{(k)}, \gamma_{g}^{(k)}, \theta_{g}^{(k)})X$, where $P_{2}(\beta_{g}^{(k)}, \gamma_{g}^{(k)}, \theta_{g}^{(k)}) = \textnormal{diag}\bigl\{\frac{(\exp(x_{i}^{\mathsf{T}}\gamma_{g}^{(k)})}{1 + \theta_{g} \exp(x_{i}^{\mathsf{T}} \gamma_{g}^{(k)})^{2}}\bigl\}$,
...
(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})^{T} = (\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})^{T} + [I_{n}(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})]^{-1}S(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k}) \tag{iv}
\end{equation}
