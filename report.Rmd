...
\begin{equation} \label{estep}
     E[z_{i,g}|\zeta_{i,g}^{k}, Y_{i,g}] = P(z_{i,g} = 1 | \zeta_{i,g}^{k}, Y_{i,g}) = \frac{\pi_{i,g}^{k}}{\pi_{i,g}^{k} + (1 - \pi_{i,g}^{k})(\frac{1}{1 + \theta_{g}^{k}\mu_{i,g}^{k}})^{\theta_{g}^{-(k)}}}I(y_{i,g}=0) \tag{$*$}
\end{equation}
...
To optimize $E[l_{c,g}|\zeta_{g}^{k}, Y_{g}]$ with respect to $\beta$, $\gamma$, and $\theta$ we implement Newton-Raphson's algorithm. Hence, the derivation of the score function and Fisher's information matrix is necessary. 

\noindent **Score Function:** We derive the score function by solving the following:

\begin{itemize}
    \item $\frac{\partial l_{c,(i,g)}}{\partial \beta_{g}} = \frac{\partial l_{c,(i,g)}}{\partial \mu_{i,g}} \frac{\partial \mu_{i,g}}{\partial \beta_{g}}$,

    \item $\frac{\partial l_{c,(i,g)}}{\partial \gamma_{g}} = \frac{\partial l_{c,(i,g)}}{\partial \pi_{i,g}} \frac{\partial \pi_{i,g}}{\partial \gamma_{g}}$, and

    \item $\frac{\partial l_{c,(i,g)}}{\partial \theta_{g}}$
\end{itemize}
...
\begin{itemize}
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \beta_{g}\beta_{g}^{T}}$,
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \beta_{g}\gamma_{g}}$,      
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \beta_{g}\theta_{g}}$,  
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \gamma_{g}\gamma_{g}^{T}}$,
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \gamma_{g}\beta_{g}}$,
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \gamma_{g}\theta_{g}}$, and  
    \item $\frac{\partial^{2} l_{c,(i,g)}}{\partial \theta_{g}^{2}}$
\end{itemize}
...
(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})^{T} = (\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})^{T} + [I_{n}(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k})]^{-1}S(\beta_{g}^{k}, \gamma_{g}^{k}, \theta_{g}^{k}) \tag{$**$}
\end{equation}
