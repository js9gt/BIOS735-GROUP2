...
We observe counts in our dataset which would typically be modeled through poisson regression. However, due to the large number of zeros, we use zero-inflated negative binomial regression instead. This dataset is high dimensional, as the number of OTUs (taxa consisting of "similar" species) exceeds the number of subjects from which data was collected from. 

The first step in the process is to identify the OTUs with significantly differential abundance to include in the logistic regression model in step 2. The idea is, that since we have so many different variable species, we essentially want to perform variable selection to identify the most significant OTUs to include in our model. 

## Step 1: Find differential abundance species using the zero inflated negative binomial (ZINB) model

\textcolor{red}{below copied from the proposal. We need to include: clear description of module 2 likelihood based methods used with likelihood, estimation procedure/hypothesis testing approach, motivation for use and justification, assumptions}
...
Suppose $Z_{i,g}$ indicates whether or not $Y_{i,g}$ belongs to the zero-inflated part. We can also use the complete data likelihood (CDL) to rewrite \ref{Eq_1}.
...
Therefore, we can use the EM algorithm to find the MLE for the $g$th OTU.
...
Using the identified OTUs from step 1, we will fit a logistic regression to predict ASD status, a binary outcome.

\textcolor{red}{what do we mean by both compositional and count data? Need to flesh out this section}
...
Finally, to compare logistic regression and random forest classification performance, ROC AUCs will be computed and compared for the two, as well as the aforementioned sensitivity and specificity calculations for both models.

## Package

\textcolor{red}{include brief package description with the relevant functions}

\textcolor{red}{Also add description of the random forest "caret" package briefly}

# Results

\textcolor{red}{First: fit our model with ZINB and Poisson regression}
```{r}
## Loading Package
library(ZINB)

## read in data
microbiome <- read.csv("~/Desktop/BIOS761-GROUP2/Data/microbiome.csv")

# eventually wrap in 10 fold CV

## fitting ZINB model
# n: number of species
# p: the actual subjects
# y: n-dimensional response vector
# X: nxp 

## LRT for multiple responses to find important predictors

## fit logistic regression with those identified predictors


## calculate accuracy, specificity, sensitivity metrics

## calculate ROC and AUC
```

\textcolor{red}{Second: fit random forest}
```{r}
## Loading Package
library(caret)
...
\textcolor{red}{Add written comparison}
