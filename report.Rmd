...
As a preface, while the original model intention was written as above, there were still too many OTU's selected by the ZINB model for the logistic regression to converge. Thus in our final analysis, we used penalized logistic regression through lasso regression, which can perform variable selection and thus further reduce the dimensions of our covariates.
...
## load function, which uses the ZINB model, fits the cross validated model, and 
# compares with random forest
source("function.R")
...
roc(result$status,as.numeric(result$pred.lasso),plot=T,percent = T,legacy.axes = T,
    print.auc = T,col = "#377eb8",
    ylab = "True positive (%)",xlab = "False Positive %",main = "ROC")
plot.roc(result$status,as.numeric(result$pred.rf),percent = T,add=T,print.auc = T,
         print.auc.y = 40,col="#4daf4a")
